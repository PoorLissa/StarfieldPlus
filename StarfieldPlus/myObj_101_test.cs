...
namespace my
{
    public class myObj_101 : myObject
    {
        ...
        public static void Process(System.Windows.Forms.Form form, ref bool isAlive)
        {
            ...
            using (Graphics gg = Graphics.FromImage(scr))
            {
                gg.CopyFromScreen(Point.Empty, Point.Empty, new Size(Width, Height));
            }
            ...
            using (Brush br = new SolidBrush(Color.Red))
            {
                ...
                while (isAlive)
                {
                    ...
                    // Normal image
                    //g.DrawImage(scr, x, y, rect, GraphicsUnit.Pixel);

                    g.DrawImage(scr, x/2 + Width/4, y/2 + Height/4, rect, GraphicsUnit.Pixel);
